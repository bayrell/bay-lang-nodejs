...
Object.assign(BayLang.LangES6.TranslatorES6Html,
{
	...
	OpHtmlAttrs: function(t, attrs, is_component)
	{
		...
		for (var attrs_i = 0; attrs_i < attrs.count(); attrs_i++)
		{
			...
			var is_event = __v3.substr(attr_key, 0, 7) == "@event:";
			if (attr_key == "class")
			{
				...
				if (attr_elem_name == "" && attr.value instanceof __v4)
				{
					...
					var arr = __v5.split(" ", attr.value.value);
		...
		if (attr_key_value != "")
		{
			res_attrs.push("\"key\":" + use("Runtime.rtl").toStr(attr_key_value));
		}
		/*
	...
	OpHtmlTag: function(t, op_code, var_name)
	{
		...
		else if (op_code instanceof __v2)
		{
			...
			else if (is_component)
			{
				...
				if (has_childs)
				{
					...
					content += use("Runtime.rtl").toStr(t.s("let " + use("Runtime.rtl").toStr(new_var_name) + use("Runtime.rtl").toStr(" = ") + use("Runtime.rtl").toStr("this._c(") + use("Runtime.rtl").toStr(var_name) + use("Runtime.rtl").toStr(", ") + use("Runtime.rtl").toStr(tag_name) + use("Runtime.rtl").toStr(", ") + use("Runtime.rtl").toStr(attrs) + use("Runtime.rtl").toStr(", ") + use("Runtime.rtl").toStr(f) + use("Runtime.rtl").toStr(");")));
					has_childs = false;
				}
				else
				{
					content += use("Runtime.rtl").toStr(t.s("/* Component '" + use("Runtime.rtl").toStr(op_code.tag_name) + use("Runtime.rtl").toStr("' */")));
					content += use("Runtime.rtl").toStr(t.s("let " + use("Runtime.rtl").toStr(new_var_name) + use("Runtime.rtl").toStr(" = ") + use("Runtime.rtl").toStr("this._c(") + use("Runtime.rtl").toStr(var_name) + use("Runtime.rtl").toStr(", ") + use("Runtime.rtl").toStr(tag_name) + use("Runtime.rtl").toStr(", ") + use("Runtime.rtl").toStr(attrs) + use("Runtime.rtl").toStr(");")));
				}
			}
			else
			{
				if (op_code.tag_name == "Teleport")
				{
					...
					content += use("Runtime.rtl").toStr(t.s("let " + use("Runtime.rtl").toStr(new_var_name) + use("Runtime.rtl").toStr(" = ") + use("Runtime.rtl").toStr("this._teleport(") + use("Runtime.rtl").toStr(var_name) + use("Runtime.rtl").toStr(", ") + use("Runtime.rtl").toStr(attrs) + use("Runtime.rtl").toStr(");")));
				}
				else
				{
					...
					content += use("Runtime.rtl").toStr(t.s("let " + use("Runtime.rtl").toStr(new_var_name) + use("Runtime.rtl").toStr(" = ") + use("Runtime.rtl").toStr("this._e(") + use("Runtime.rtl").toStr(var_name) + use("Runtime.rtl").toStr(", ") + use("Runtime.rtl").toStr(tag_name) + use("Runtime.rtl").toStr(", ") + use("Runtime.rtl").toStr(attrs) + use("Runtime.rtl").toStr(");")));
				}
			}
			if (has_childs)
			{
		...
		else
		{
			...
			t = Runtime.rtl.setAttr(t, Runtime.Collection.from(["save_op_codes"]), save_op_codes);
			/*t <= save_op_code_inc <= save_op_code_inc;*/
			content += use("Runtime.rtl").toStr(t.s("/* Render */"));
			content += use("Runtime.rtl").toStr(t.s("this._t(" + use("Runtime.rtl").toStr(var_name) + use("Runtime.rtl").toStr(", ") + use("Runtime.rtl").toStr(item_value) + use("Runtime.rtl").toStr(");")));
		}
	...
	{
		var content = "";
		content += use("Runtime.rtl").toStr(t.s("let __v = [];"));
