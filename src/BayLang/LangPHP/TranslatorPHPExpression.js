...
Object.assign(BayLang.LangPHP.TranslatorPHPExpression,
{
	...
	findModuleNames: function(t, names)
	{
		if (names.count() > 0)
		{
			...
			{
				var __v0 = use("Runtime.rs");
				names = __v0.split(".", module_name).concat(names.slice(1));
			}
		}
		return names;
	},
	...
	{
		var __v0 = use("Runtime.rs");
		return "\\" + use("Runtime.rtl").toStr(__v0.join("\\", this.findModuleNames(t, names)));
	},
	/**
	 * OpTypeIdentifier
	 */
	OpTypeIdentifier: function(t, op_code)
	{
		...
		var __v0 = use("Runtime.rs");
		var s = "\\" + use("Runtime.rtl").toStr(__v0.join("\\", names));
	...
	OpDict: function(t, op_code, flag_array)
	{
		...
		{
			var __v1 = use("Runtime.rs");
			content = "[" + use("Runtime.rtl").toStr(__v1.join(",", values)) + use("Runtime.rtl").toStr("]");
		}
		return use("Runtime.Vector").from([t,content]);
	},
	/**
	...
	Dynamic: function(t, op_code, next_op_code)
	{
		...
		var __v21 = use("BayLang.OpCodes.OpCurry");
		var __v22 = use("BayLang.OpCodes.OpCall");
		...
		else if (op_code instanceof __v1)
		{
			...
			else if (op_code_next instanceof __v8)
			{
				...
				{
					prev_kind = "var";
					var res = t.constructor.addSaveOpCode(t, use("Runtime.Map").from({"var_content":t.pipe_var_name + use("Runtime.rtl").toStr("->val")}));
					t = Runtime.rtl.attr(res, 0);
					s = Runtime.rtl.attr(res, 1);
					prev_kind = "static";
				}
				else
				{
					...
					if (t.modules.has(op_code_next.value) || op_code_next.kind == __v11.KIND_SYS_TYPE)
					{
						prev_kind = "static";
					}
				}
			...
			{
				var res = t.constructor.addSaveOpCode(t, use("Runtime.Map").from({"var_content":s}));
				t = Runtime.rtl.attr(res, 0);
				s = Runtime.rtl.attr(res, 1);
			}
			...
			for (var i = 0; i < attrs.count(); i++)
			{
				...
				var __v17 = use("BayLang.OpCodes.OpAttr");
				var __v18 = use("BayLang.OpCodes.OpAttr");
				if (attr.kind == __v12.KIND_ATTR)
				{
					/* Pipe */
					var __v13 = use("BayLang.OpCodes.OpCall");
					if (t.is_pipe && !(next_op_code instanceof __v13))
					{
						if (i == attrs_sz - 1)
						{
				...
				else if (attr.kind == __v14.KIND_STATIC)
				{
					if (prev_kind == "static")
					{
						var attr_val = attr.value.value;
						var __v15 = use("BayLang.OpCodes.OpCall");
						var __v16 = use("Runtime.rs");
						if (i == attrs_sz - 1 && next_op_code instanceof __v15)
						{
							s += use("Runtime.rtl").toStr("::" + use("Runtime.rtl").toStr(attr_val));
						}
						else if (__v16.upper(attr_val) == attr_val)
						{
							s += use("Runtime.rtl").toStr("::" + use("Runtime.rtl").toStr(attr_val));
						}
						else
				...
				else if (attr.kind == __v17.KIND_DYNAMIC)
				{
					var res = this.Expression(t, attr.value);
					t = Runtime.rtl.attr(res, 0);
					/* s ~= "[" ~ res[1] ~ "]"; */
					s = "\\Runtime\\rtl::attr($ctx, " + use("Runtime.rtl").toStr(s) + use("Runtime.rtl").toStr(", ") + use("Runtime.rtl").toStr(Runtime.rtl.attr(res, 1)) + use("Runtime.rtl").toStr(")");
				}
				else if (attr.kind == __v18.KIND_DYNAMIC_ATTRS)
				{
					var __v19 = use("Runtime.Vector");
					var items = new __v19();
					if (attr.attrs != null)
					{
					...
					s = "\\Runtime\\rtl::attr($ctx, " + use("Runtime.rtl").toStr(s) + use("Runtime.rtl").toStr(", [") + use("Runtime.rtl").toStr(__v20.join(", ", items)) + use("Runtime.rtl").toStr("])");
				}
			}
			return use("Runtime.Vector").from([t,s]);
		}
		else if (op_code instanceof __v21)
		{
		...
		else if (op_code instanceof __v22)
		{
	...
	OpMath: function(t, op_code)
	{
		...
		else
		{
			...
			if (op_math == "~")
			{
				content = op1 + use("Runtime.rtl").toStr(" . ") + use("Runtime.rtl").toStr(this.rtlToStr(t, op2));
			}
	...
	OpMethod: function(t, op_code)
	{
		...
		var content = "new \\Runtime\\Callback(" + use("Runtime.rtl").toStr(val1) + use("Runtime.rtl").toStr(", ") + use("Runtime.rtl").toStr(this.toString(val2)) + use("Runtime.rtl").toStr(")");
	...
	OpCurry: function(t, op_code)
	{
		...
		for (var i = 0; i < args_sz; i++)
		{
			...
			var __v3 = use("BayLang.OpCodes.OpCurryArg");
			if (arg instanceof __v3)
			{
				continue;
			}
			var __v4 = use("BayLang.OpCodes.OpIdentifier");
			if (arg instanceof __v4)
			{
				args_use.push("$" + use("Runtime.rtl").toStr(arg.value));
			}
		}
		...
		for (var i = 0; i < args_sz; i++)
		{
			...
			var arr_use = new __v5();
			arr_use.appendItems(args_use);
			for (var j = 0; j < i; j++)
			{
				var arg_use = args.item(j);
				arr_use.push("$__varg" + use("Runtime.rtl").toStr(arg_use.pos));
			}
			if (use_obj_item != "")
			{
			...
			if (arr_use.count() > 0)
			{
				...
				s_use = " use (" + use("Runtime.rtl").toStr(__v6.join(", ", arr_use)) + use("Runtime.rtl").toStr(")");
			}
			if (args_sz - 1 == i)
			{
		...
		for (var i = 0; i < op_code.args.count(); i++)
		{
			s = "";
			var item = op_code.args.item(i);
			var __v7 = use("BayLang.OpCodes.OpCurryArg");
			if (item instanceof __v7)
			{
	...
	OpPipe: function(t, op_code, is_expression)
	{
		...
		var res = t.constructor.addSaveOpCode(t, use("Runtime.Map").from({"content":t.s(var_name + use("Runtime.rtl").toStr(" = new \\Runtime\\Monad(") + use("Runtime.rtl").toStr(value) + use("Runtime.rtl").toStr(");"))}));
		t = Runtime.rtl.attr(res, 0);
		/* Output items */
	...
	OpTypeConvert: function(t, op_code)
	{
		...
		content = "\\Runtime\\rtl::to(" + use("Runtime.rtl").toStr(value) + use("Runtime.rtl").toStr(", ") + use("Runtime.rtl").toStr(this.toPattern(t, op_code.pattern)) + use("Runtime.rtl").toStr(")");
		return use("Runtime.Vector").from([t,content]);
	},
	/**
	 * OpDeclareFunction
	...
	OpDeclareFunction: function(t, op_code)
	{
		...
		if (op_code.vars != null && op_code.vars.count() > 0)
		{
			...
			var __v0 = use("Runtime.rs");
			content += use("Runtime.rtl").toStr(" use (" + use("Runtime.rtl").toStr(__v0.join(",", vars)) + use("Runtime.rtl").toStr(")"));
		}
