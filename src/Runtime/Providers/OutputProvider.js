...
{
	use("Runtime.BaseProvider").call(this);
...
Object.assign(Runtime.Providers.OutputProvider.prototype,
{
	...
	print: function(message, new_line, type)
	{
		...
		if (!__v0.isString(message))
		{
			var __v1 = use("Runtime.Exceptions.RuntimeException");
			throw new __v1("print message must be string")
		}
	...
	color: function(color, message)
	{
		...
		message = __v0.chr(27) + use("Runtime.rtl").toStr("[") + use("Runtime.rtl").toStr(color) + use("Runtime.rtl").toStr("m") + use("Runtime.rtl").toStr(message);
		var __v1 = use("Runtime.rs");
		message = message + use("Runtime.rtl").toStr(__v1.chr(27)) + use("Runtime.rtl").toStr("[0m");
		return message;
	},
	/**
